function [map,precision_at_k,recall_at_k] = precision(trn_label, trn_binary, tst_label, tst_binary, top_k, mode)   
K = top_k;
...
for i = 1:QueryTimes
    ...
    all_true=0;
    for j=1:size(trn_binary,2)
        if (query_label==trn_label(j))
            
            all_true = all_true + 1;
        end
    end
    RC(i)=total_relevant / all_true;
