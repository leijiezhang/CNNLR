...
caffe.reset_all();            %来清理你所创建的所有的solvers，和stand-alone nets。
...
% Set caffe mode
if exist('use_gpu', 'var') && use_gpu    %hecks only for variables.
  caffe.set_mode_gpu();
...
if ~exist(net_weights, 'file')   %hecks for files or directories.
...
% load mean file
d = load('/home/leijzhan/my_caffe/matlab/+caffe/imagenet/ilsvrc_2012_mean.mat');
mean_data = d.mean_data;

batch_size = 20;

% prepare input
num_images = length(list_im);     %test num
scores = zeros(dim,num_images,'single');      %single is a type
num_batches = ceil(length(list_im)/batch_size);  
initic=tic;  % tic Start a stopwatch timer.  tic and TOC functions work together to measure elapsed time.
for bb = 1 : num_batches
    batchtic = tic;
    range = 1+batch_size*(bb-1):min(num_images,batch_size * bb); %in every batch ,image num
    tic
